...
If we fix the affine map $\mathcal{T}$, the public key composition represents a linear relation between the coefficients $q_{ij}^{(k)}, f_{ij}^{(k)}$ of the monomial $x_{i} x_{j}$ in $\mathcal{Q}$ and $\mathcal{F}$ respectively, with the form
...
\end{cases},
\end{equation}
and $k \in \{v_{1}, \dots, n\}, \; i, j \in \{1, \dots, n\}$. Eq.~\ref{eq:1} can be represented in a more compact form, since $f_{rs}^{(k)} = 0$ if $r, s \in O_{\ell}$.
