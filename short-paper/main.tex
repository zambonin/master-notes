...
\end{equation}
Let $D_{w} = v_{w}(v_{w} + 1)/2 + v_{w} o_{w}$ be the number of quadratic terms in the central polynomials of the $w$-th layer, and $D = n(n + 1)/2$ be the number of quadratic terms in the public polynomials. Further, define a monomial ordering that acts as following. The $D_{w}$ monomials that appear only in the central polynomials of the $w$-th layer shall appear before the $D_{w + 1}$ monomials, with $w \in \{1, \dots, u\}$. The linear and constant monomials come last. Lexicographic ordering is used to order the elements of these ``blocks'' themselves.


Eq.~\ref{eq:2} can be represented in a more compact form by creating a $D_{2} \times D_{2}$ matrix $A$ with the coefficients $\alpha_{ij}^{rs}$.
