...
We describe below the Rainbow digital signature scheme due to Ding and Schmidt~\cite{Ding:inproc:2005:jun}. Consider a finite field $\mathbb{F}_{q}$, and a sequence of positive integers $0 = v_{0} < v_{1} < v_{2} < \dots < v_{u+1} = n$. For $i = \{1, \dots, u\}$, let $V_{i} = \{1, \dots, v_{i}\}$ and $O_{i} = \{v_{i} + 1, \dots, v_{i+1}\}$.
