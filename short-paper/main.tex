...
We describe below the Rainbow digital signature scheme due to Ding and Schmidt~\cite{Ding:inproc:2005:jun}. Consider a finite field $\mathbb{F}_{q}$, and a sequence of positive integers $0 = v_{0} < v_{1} < v_{2} < \dots < v_{u+1} = n$, with $m = n - v_{1}$. For $i = \{1, \dots, u\}$, let $V_{i} = \{1, \dots, v_{i}\}$ and $O_{i} = \{v_{i} + 1, \dots, v_{i+1}\}$.

\emph{Key generation.}

The private key is a triple of invertible maps. We randomly generate linear or affine maps $\mathcal{S}: \mathbb{F}_{q}^{m} \rightarrow \mathbb{F}_{q}^{m}$ and $\mathcal{T}: \mathbb{F}_{q}^{n} \rightarrow \mathbb{F}_{q}^{n}$. The central map $\mathcal{F}: \mathbb{F}_{q}^{m} \rightarrow \mathbb{F}_{q}^{n}$
has a special structure. It consists of $m$ quadratic polynomials $f^{(v_{1} + 1}, \dots, f^{(n)}$ of the form
