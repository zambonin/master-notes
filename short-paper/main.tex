...
\end{equation}
Choose at random a map $\mathcal{S}$ with the additional restriction that $S_{22}$ must be invertible. Also generate randomly a map $\mathcal{T}$ and compute $A$ through Eq.~\ref{eq:2}. If $A$ or $A_{11}$ are not invertible, choose another $\mathcal{T}$.
