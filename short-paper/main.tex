...
With this strategy, we can configure the $m \times D_{1}$ matrix $B_{1}$ and the $o_{2} \times (D_{2} - D_{1})$ matrix $B_{2}$ with elements that generate a succinct structure. Randomly choose vectors $a^{(1)} \in \mathbb{F}_{q}^{D_{1}}, a^{(2)} \in \mathbb{F}_{q}^{D_{2} - D_{1}}$, and set $b_{ij}^{(1)} = a_{((j - i) \pmod{D_{1}}) + 1}^{(1)}, b_{ij}^{(2)} = a_{((j - i) \pmod{D_{2} - D_{1}}) + 1}^{(1)}$
