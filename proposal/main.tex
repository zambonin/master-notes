...
utilizado para a autenticação entre estas.

Devido à construção do algoritmo, e como os processos de geração de chaves,
assinatura e verificação são realizados, demasiadas\footnote{Assumindo $w = 16$ e um parâmetro de segurança $n = 256$, e a vida útil para a mensagem de um ano, o número total de aplicações da função de resumo criptográfico é de $\mathbf{}$ para o esquema Winternitz convencional.} aplicações da função
